...
def train(epoch,logger,model,model_min,model_min_acc,device,optimizer,estim,valid_loss_min,valid_accuracy_max,loader,valid_loader,n_glob,nl_glob,save_path,save_name,loader_u=None,prob=None,method=None,lamb=None,p_in=None,nlab_perclass=None,cutoff_meth="CADR",beta_cutoff=0.5,info="not_known",n_classes=10,epoch_loss_min=0,epoch_accuracy_max=0,train_loader=None):
    ...
    if estim == "oracle":
        ...
        for (im_l, target_l), (im_u, target_u) in zip(loader, loader_u):
            ...
            loss_l = cl_loss.loss_supervised_MCAR(pred_l,pred_u,target_l)
            loss_u = cl_loss.loss_unsupervised_MCAR_biased(pred_l,pred_u,target_l)
        ...
        for (im_l, target_l), (im_u, target_u) in zip(loader, loader_u):
            ...
            prediction_l = torch.tensor(nlab_perclass).to(device)
            ...
            if info=="not_known":
                ...
                p_y = p_in 
                prob = (1/p_y)*torch.tensor(nlab_perclass).to(device)/n_glob
        ...
        for (im_l, target_l), (im_u, target_u) in zip(loader, loader_u):
            ...
            p_y = (1/prob)*(nlab_perclass/n_glob)
            p_in = p_y
        ...
        for (im_l, target_l), (im_u, target_u) in zip(loader, loader_u):
            ...
            if info=="not_known":
                ...
                p_in = update_p
                prob = (1/p_in)*torch.tensor(nlab_perclass).to(device)/n_glob
